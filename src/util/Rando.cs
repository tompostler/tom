...
{
    /// <summary>
    /// Provide novel random generators (name, 8ball) or secure random number generation.
    /// </summary>
    /// <remarks>
    ...
    public static class Rando
    {
        ...
        /// </summary>
        public static ulong GetNumeric(RandomType type)
        {
            return type switch
            {
                RandomType.Bit => Generate(1),
                RandomType.Crumb => Generate(2),
                RandomType.Nibble => Generate(4),
                RandomType.Byte => Generate(8),
                RandomType.Short => Generate(16),
                RandomType.Int => Generate(32),
                RandomType.Long => Generate(64),
                _ => throw new NotImplementedException($"{type} was not implemented.")
            };
        }

        /// <summary>
        /// Get anything from this class, but as a string.
        /// </summary>
        public static string GetString(RandomType type)
        {
            object response = type switch
            {
                RandomType.EightBall => EightBallResponses[(int)Generate(8) % EightBallResponses.Length],
                RandomType.Guid => Guid.NewGuid().ToString("D"),
                RandomType.Name => nameAdjectives[(int)Generate(8) % nameAdjectives.Length] + '_' + nameNames[(int)Generate(8) % nameNames.Length],
                RandomType.Bit => Generate(1),
                RandomType.Crumb => Generate(2),
                RandomType.Nibble => Generate(4),
                RandomType.Byte => Generate(8),
